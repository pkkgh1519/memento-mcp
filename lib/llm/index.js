...
export async function llmJson(prompt, options = {}) {
  ...
  const chain       = await buildChain(startedAt);
  ...
  for (const provider of chain) {
    ...
    if (LLM_CONCURRENCY_ENABLED) {
      ...
      const waitTimeoutMs    = remainingMs === null ? LLM_CONCURRENCY_WAIT_MS : Math.max(1, Math.min(LLM_CONCURRENCY_WAIT_MS, remainingMs));
      const waitStart        = Date.now();
      try {
        await sem.acquire(waitTimeoutMs);
      } catch (_err) {
        const postWaitRemainingMs = getRemainingChainMs(startedAt);
        if (postWaitRemainingMs !== null && postWaitRemainingMs <= 0) {
          errors.push(`chain deadline exceeded after ${LLM_CHAIN_TIMEOUT_MS}ms`);
          break;
        }
      ...
      try {
        ...
        const providerOptions = withProviderTimeoutCap(options, provider, postWaitRemainingMs);
