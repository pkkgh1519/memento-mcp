...
/**
 * 개별 스테이지 실행 시간 및 결과를 계측한다.
 *
 * @param {string}            name - 스테이지 이름
 * @param {() => Promise<*>}  fn   - 실행할 비동기 함수
 * @returns {Promise<{name: string, durationMs: number, affected: number, status: string, error?: string}>}
 */
async function timedStage(name, fn) {
  const start = performance.now();
  try {
    const result = await fn();
    const affected = (result !== null && result !== undefined && result !== false)
      ? (typeof result === "number" ? result : (Array.isArray(result) ? result.length : 1))
      : 0;
    return { name, durationMs: Math.round(performance.now() - start), affected, status: "ok" };
  } catch (err) {
    return { name, durationMs: Math.round(performance.now() - start), affected: 0, status: "error", error: err.message };
  }
}

const SCHEMA = "agent_memory";

const CONSOLIDATION_QUEUE = {
...
export class MemoryConsolidator {
   ...
   *   진행 이벤트 콜백. 각 스테이지 완료 시 호출된다. 제공되지 않으면 no-op.
   * @returns {Promise<Object>} 작업 결과 요약
   */
  async consolidate(onProgress = null) {
    const queued      = CONSOLIDATION_QUEUE.running || CONSOLIDATION_QUEUE.waiting > 0;
    const waitStartMs = Date.now();
    CONSOLIDATION_QUEUE.waiting += 1;

    const previous = CONSOLIDATION_QUEUE.tail.catch(() => {});
    let releaseSlot;
    const slot = new Promise(resolve => { releaseSlot = resolve; });
    CONSOLIDATION_QUEUE.tail = previous.then(() => slot);

    await previous;
    CONSOLIDATION_QUEUE.waiting -= 1;
    CONSOLIDATION_QUEUE.running = true;
    const queueWaitMs = Date.now() - waitStartMs;

    try {
      if (queued) {
        logInfo("[MemoryConsolidator] Consolidation request queued behind active run");
      }
      const result = await this._runConsolidationCycle(onProgress);
      return {
        ...result,
        queued,
        queueWaitMs
      ...
      releaseSlot();
      CONSOLIDATION_QUEUE.running = false;
    }
  }

  async _runConsolidationCycle(onProgress = null) {
    ...
    /**
     * 스테이지를 실행하고 push 후 progress 이벤트를 발행한다.
     *
     * @param {string}           name
     * @param {() => Promise<*>} fn
    ...
    const runStage = async (name, fn) => {
      const s = await timedStage(name, fn);
      stages.push(s);
      emit({
        phase    : name,
    ...
    try {
      ...
      /** 1. TTL 계층 전환 (전환 수 추적) */
      const s1 = await runStage("ttl_transition", () => this._transitionWithCount());
      results.ttlTransitions = s1.affected;

      /** 2. 중요도 감쇠 */
      const s2 = await runStage("importance_decay", async () => {
        await this.store.decayImportance();
        return 1;
      });
      results.importanceDecay = s2.status === "ok";

      /** 3. 만료 파편 삭제 */
      const s3 = await runStage("expired_delete", () => this.store.deleteExpired());
      results.expiredDeleted = s3.affected;

      /** 3.5. GC 후보 분포 미리보기 */
      const s35 = await runStage("gc_preview", async () => {
        ...
        return gcPreview.rows.length;
      });
      if (s35.status === "error") {
        logWarn(`[MemoryConsolidator] GC preview query failed: ${s35.error}`);
        results.gcCandidatesByType = {};
      }

      /** 3.7. 긴 파편 분할 (Gemini CLI) */
      const s37 = await runStage("split_long_fragments", () => this._splitLongFragments());
      results.fragmentsSplit = s37.affected;

      /** 4. 중복 파편 병합 */
      const s4 = await runStage("merge_duplicates", () => this._mergeDuplicates());
      results.duplicatesMerged = s4.affected;

      /** 4.5. 시맨틱 중복 제거 (cos >= 0.92, topic별 KNN) */
      const s45 = await runStage("semantic_dedup", () => this._semanticDedup());
      results.semanticDedupMerged = s45.affected;

      /** 4.7. 오래된 파편 압축 (cos >= 0.80, importance < 0.5) */
      const s47 = await runStage("compress_old_fragments", () => this._compressOldFragments());
      results.compressedFragments = s47.affected;

      /** 5. 누락 임베딩 보충 */
      const s5 = await runStage("embeddings_backfill", async () => {
        const _embWorker = new EmbeddingWorker();
        return _embWorker.processOrphanFragments(5);
      });
      results.embeddingsAdded = s5.affected;

      /** 5.5. 소급 자동 링크 (고립 파편 연결) */
      const s55 = await runStage("retro_link", async () => {
        ...
        return retroResult.linksCreated;
      });

      /** 6. utility_score 갱신 */
      const s6 = await runStage("utility_score_update", () => this._updateUtilityScores());
      results.utilityUpdated = s6.affected;

      /** 6.2. 고-EMA 저-importance 파편 재평가 큐 등록 */
      const s62 = await runStage("requeue_high_ema", () => this._requeueHighEmaLowQuality());
      if (s62.affected > 0) {
        logInfo(`[MemoryConsolidator] ${s62.affected}개 파편 재평가 큐 등록`);
      }

      /** 6.5. 자동 앵커 승격 (Phase 3) */
      const s65 = await runStage("promote_anchors", () => this._promoteAnchors());
      results.anchorsPromoted = s65.affected;

      /** 7. 증분 모순 탐지 (NLI + Gemini CLI 하이브리드) */
      let contraResult = { found: 0, nliResolved: 0, nliSkipped: 0 };
      const s7 = await runStage("detect_contradictions", async () => {
        contraResult = await this._detectContradictions();
        return contraResult.found ?? 0;
      });
      ...
      const s77 = await runStage("detect_supersessions", () => this._detectSupersessions());
      results.supersessionsDetected = s77.affected;

      /** 7.5. 보류 중인 모순 후처리 */
      const s75 = await runStage("process_pending_contradictions", () => this._processPendingContradictions());
      results.pendingContradictions = s75.affected;

      /** 8. 피드백 리포트 생성 */
      const s8 = await runStage("feedback_report", () => this._generateFeedbackReport());
      results.feedbackReportGenerated = s8.status === "ok" && s8.affected > 0;

      /** 8.5. 피드백 적응형 importance 보정 */
      const s85 = await runStage("feedback_calibration", () => this._calibrateByFeedback());
      if (s85.affected > 0) {
        logWarn(`[MemoryConsolidator] Feedback calibration: ${s85.affected} fragments updated`);
      }

      /** 9. Redis 인덱스 정리 */
      const s9 = await runStage("prune_keyword_indexes", async () => {
        await this.index.pruneKeywordIndexes();
        return 1;
      });
      results.indexesPruned = s9.status === "ok";

      /** 10. stale 파편 목록 수집 */
      let staleList = [];
      const s10 = await runStage("collect_stale_fragments", async () => {
        staleList = await this._collectStaleFragments();
        return Array.isArray(staleList) ? staleList.length : 0;
      });
      results.stale_fragments = staleList;

      /** 11. session_reflect 노이즈 정리 */
      const s11 = await runStage("purge_stale_reflections", () => this._purgeStaleReflections());
      results.reflectionsPurged = s11.affected;

      /** 12. search_events 30일 초과 레코드 정리 */
      const s12 = await runStage("gc_search_events", () => this._gcSearchEvents());
      results.searchEventsGarbageCollected = s12.affected;
