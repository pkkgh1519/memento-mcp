 ...
 * 작성자: 최진호
 * 작성일: 2026-02-23
 * 수정일: 2026-03-03 (RRF 하이브리드 검색 적용)
 *
  ...
  async search(query) {
    ...
    if (query.text && OPENAI_API_KEY) {
      const [l2Results, l3Results] = await Promise.all([
        this._searchL2(query, l1Ids, agentId),
        this._searchL3(query.text, agentId)
      ]);
      ...
      combined = mergeRRF(l1Ids, l2Results, l3Results, MEMORY_CONFIG.rrfSearch.k);
    } else {
    ...
    /** 중복 제거 */
    const unique = this._deduplicate(combined, query.fragmentCount || 0);
 ...
 *
 * @param {string[]} l1Ids     - Redis 결과 ID 배열 (최우선 — 가중치 2배)
 ...
 */
export function mergeRRF(l1Ids, l2Results, l3Results, k = 60) {
  const scoreMap = new Map();

  /** L1: Redis 컨텍스트 — 가중치 2배 (최신 세션 컨텍스트 우선) */
  l1Ids.forEach((id, rank) => {
    scoreMap.set(id, { id, _rrfScore: 2 / (k + rank + 1) });
  });
