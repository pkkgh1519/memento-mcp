...
export class FragmentStore {
  ...
  async deleteExpired() {
    ...
    const utilityThreshold   = gc.utilityThreshold || 0.15;
    const gracePeriodDays    = gc.gracePeriodDays || 7;
    const inactiveDays       = gc.inactiveDays || 60;
    const maxDelete          = gc.maxDeletePerCycle || 50;
    const fdPolicy           = gc.factDecisionPolicy || {};
    const fdImportance       = fdPolicy.importanceThreshold || 0.2;
    const fdOrphanDays       = fdPolicy.orphanAgeDays || 30;
      ...
      `DELETE FROM ${SCHEMA}.fragments
       WHERE id IN (
         ...
         WHERE ttl_tier NOT IN ('permanent')
           AND is_anchor = FALSE
           AND created_at < NOW() - INTERVAL '${gracePeriodDays} days'
           AND (
             (utility_score < ${utilityThreshold}
              AND (accessed_at IS NULL OR accessed_at < NOW() - INTERVAL '${inactiveDays} days')
             )
             OR
             (type IN ('fact', 'decision')
              AND importance < ${fdImportance}
              AND access_count = 0
              ...
              AND created_at < NOW() - INTERVAL '${fdOrphanDays} days'
             )
             OR
             (importance < 0.1
