  ...
  async _autoLinkSessionFragments(fragments, agentId = "default") {
    ...
    for (const err of errors) {
      for (const dec of decisions) {
        ...
        await this.store.createLink(err.id, dec.id, "caused_by", agentId).catch((e) => { logWarn(`[MemoryManager] auto-link creation failed: ${e.message}`); });
      }
    }
    ...
    for (const proc of procedures) {
      for (const err of errors) {
        ...
        await this.store.createLink(proc.id, err.id, "resolved_by", agentId).catch((e) => { logWarn(`[MemoryManager] auto-link creation failed: ${e.message}`); });
      }
    }
  }
