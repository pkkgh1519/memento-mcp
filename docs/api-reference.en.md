...
v3.1.0 **removed** the top-level `_searchEventId` / `_memento_hint` / `_suggestion` mirror fields. The same values are available only at `_meta.searchEventId` / `_meta.hints` / `_meta.suggestion`. Clients must use the `_meta` path exclusively.

`_meta.suggestion` (v2.9.0+): A hint object generated by the RecallSuggestionEngine based on analysis of the current search pattern. `null` when no issue is detected. (Mirror was also available at top-level `_suggestion` through v3.0.0; removed in v3.1.0.)
