...
- Quick Start: 최소 의존성으로 서버를 띄우는 경로
- First Memory Flow: 첫 `remember`, `recall`, `context` 성공 검증
- Troubleshooting: 대표 설치/실행 오류 해결
- Windows WSL2 Setup: Windows에서 가장 안정적인 설치 경로
- Windows PowerShell Setup: Bash 없이 수동으로 설치하는 제한 경로
- Claude Code Configuration: Claude Code에서 memento를 MCP 서버로 등록하는 방법
