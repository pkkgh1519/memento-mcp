...
MemoryManager는 259줄의 thin facade다. 비즈니스 로직은 `lib/memory/processors/` 하위 4개 프로세서로 위임된다.
...
```
remember(params)
  ├── dryRun 분기 — atomic 가드 선언 직전 early return
...
`dryRun` 분기는 atomic 가드 선언 직전(변수 `fragment` TDZ 이전)에 위치한다. atomic 분기는 fragment 선언 이후에 위치하여 TDZ를 해소하며, dryRun 분기도 그 이후 위치를 따른다.
